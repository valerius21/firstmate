...
GitLab projects, including self-hosted instances, are supported alongside GitHub: while any clone under `projects/` has a GitLab origin, bootstrap also requires the GitLab CLI and reports `MISSING: glab (install: brew install glab  # or the platform's package manager)`.
For this check any origin host other than literally `github.com` is treated as a GitLab origin, so a home whose only such origins are GitHub SSH aliases such as `git@github-work:owner/repo.git` or other forges sees one `MISSING: glab` line per session start, which it may ignore.
A home with only `github.com`, file, or no origins is never asked for it.
Once glab is installed, the deferred network stage runs `glab auth status --hostname <host>` for each distinct GitLab origin host and reports `NEEDS_GLAB_AUTH: <host>` for each one whose login fails, bounded at 15 seconds so it never prompts or hangs; an unreachable instance fails the same way, like `gh` offline.
Installing glab does not silence the alias case: that home then sees one `NEEDS_GLAB_AUTH: <alias>` line per session start instead, which it may ignore because the host is not a GitLab instance.
Authenticate each instance with `glab auth login --hostname <host>`, set `GITLAB_HOST=<host>` for `glab api` calls, and give HTTPS clones `credential.helper='!glab auth git-credential'` when the instance offers no SSH.
