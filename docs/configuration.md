...
For this check any origin host other than literally `github.com` is treated as a GitLab origin, so a home whose only such origins are GitHub SSH aliases such as `git@github-work:owner/repo.git` or other forges sees one `MISSING: glab` line per session start, which it may ignore or silence by installing glab.
A home with only `github.com`, file, or no origins is never asked for it.
Once glab is installed, the deferred network stage runs `glab auth status --hostname <host>` for each distinct GitLab origin host and reports `NEEDS_GLAB_AUTH: <host>` for each one whose login fails, bounded by `FM_GLAB_AUTH_TIMEOUT` seconds (default 15) so it never prompts or hangs; an unreachable instance fails the same way, like `gh` offline.
