...
- `MISSING: <tool> (install: <command>)` - list the missing tools to the captain with a one-line purpose each plus the printed install commands, wait for consent (one approval may cover the list), then run `bin/fm-bootstrap.sh install <approved tools...>`.
  ...
  For `glab`, bootstrap requires it only while a clone under `projects/` has a GitLab origin; after the install, the next session start probes each instance login and reports any gap as `NEEDS_GLAB_AUTH` below, so GitLab work is not dispatched until that login exists.
